...
async def run_basic_analysis(asset_data: dict, macro_data: dict, mep_data: dict, mep_comparison: dict) -> dict:
    ...
    try:
        response = client.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": "Sos un analista financiero especializado en el mercado argentino. Respondés siempre en JSON válido."},
...
def _fallback_basic_analysis(asset_data):
    return {
        ...
        "factors": [{"title": "Configuración pendiente", "description": "Completá OPENAI_API_KEY en .env", "type": "neutral"}],
        "mep_comparison": {"days_30":0,"days_90":0,"days_365":0,"mep_30":0,"mep_90":0,"mep_365":0},
        "summary": f"Para obtener el análisis de {asset_data['ticker']}, configurá tu clave de OpenAI en .env",
    }
...
def _fallback_deep_analysis(asset_data):
    ...
    base.update({
        ...
        "peers": [], "news": [],
        "technical_summary": "Configurá OPENAI_API_KEY para obtener análisis técnico.",
    })
    return base
